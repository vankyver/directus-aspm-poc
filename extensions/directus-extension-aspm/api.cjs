...
class GitlabClient {
  ...
  githubToken;
  headers = {};
  constructor(githubToken, githubOrg = "webgoat") {
  ...
  async loadRepositories(page, perPage = 100) {
    return axios$1.get(GITHUB_URL + `/orgs/${this.githubOrg}/repos?page=${page}per_page=${perPage}`, {
      headers: this.headers
    }).then((response) => {
      return response.data;
    });
  }
  async loadCommits(repository, perPage = 100) {
    return axios$1.get(GITHUB_URL + `/repos/${this.githubOrg}/${repository.name}/commits?&per_page=${perPage}`, {
      headers: this.headers
    }).then((response) => {
      return response.data;
    });
...
const useService = async (serviceName, context) => {
  const { database, services, accountability, getSchema } = context;
  const { ItemsService: ItemsService2 } = services;
  const schema = await getSchema();
  return new ItemsService2(serviceName, { accountability, database, schema });
};

const PER_PAGE = 200;
var e0 = defineOperationApi({
  id: "github-loader",
  handler: async function(_, context) {
    ...
    const githubClient = new GitlabClient(context.env.GITHUB_TOKEN);
    logger.info("[Github Loader] Start loading repositories");
    for (let page = 1; page < 2; page++) {
      logger.info(`[Github Loader] page - ${page}`);
      try {
        const repositories = await githubClient.loadRepositories(page, PER_PAGE);
        ...
        for (const repository of repositories) {
          ...
          const commits = await githubClient.loadCommits(repository, 10);
...
const hooks = [];const endpoints = [];const operations = [{name:'github-loader',config:e0},{name:'defect-writer',config:e1}];
